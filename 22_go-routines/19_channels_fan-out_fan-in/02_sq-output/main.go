...
func merge(cs ...chan int) chan int {
	...
	for _, c := range cs {
		go func(ch chan int) {
			...
			wg.Done()
			// here we are passing in c as an argument to the anonymous self executing go func
			// that way it creates a closure with the appropriate value
			// otherwise when it launches off to process in the nether
			// it could end up reading an inappropriate value from the the for loop that launched it
		}(c)
	}
...
/*
CHALLENGE #1
We you know HOW to do fan out/fan in, but do we know WHY?
