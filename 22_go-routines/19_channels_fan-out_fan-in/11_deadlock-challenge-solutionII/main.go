...
func fanOut(in <-chan int, n int) []<-chan int {
	// xc := make([]<-chan int, n)
	// the above code was the mistake, we initialized the slice with an underlying
	// array of size n, then below started appending to it, increasing its size
	// while leaving the initialized internal values as zero
	var xc []<-chan int
	for i := 0; i < n; i++ {
